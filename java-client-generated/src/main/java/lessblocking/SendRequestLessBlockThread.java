...
public class SendRequestLessBlockThread extends AbsSendRequestThread implements Runnable{
    ...
    while (this.numTakenReqs.get() < LoadTestConfig.NUM_TOTAL_REQUESTS) {
      ...
      while (curRoundNumOfSuccessfulReqs < taken) {
        boolean isSuccess = this.sendSingleRequest(
            RequestGenerator.generateSingleRequest(), swipeApi, this.numSuccessfulReqs, this.numFailedReqs);
        if (isSuccess) {
   ...
   */
  @Override
  public boolean sendSingleRequest(Request request, SwipeApi swipeApi, AtomicInteger numSuccessfulReqs, AtomicInteger numFailedReqs) {
