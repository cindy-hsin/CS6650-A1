...
public class SendRequestAverageThread extends AbsSendRequestThread implements Runnable{
  ...
  public void run() {
    ...
    for (int i = 0; i < LoadTestConfig.NUM_TOTAL_REQUESTS / LoadTestConfig.NUM_THREADS; i++) {
      Record record = this.sendSingleRequestWithRecord(RequestGenerator.generateSingleRequest(), swipeApi, this.numSuccessfulReqs, this.numFailedReqs);
      records.add(record);
    }
  ...
  private Record sendSingleRequestWithRecord(Request request, SwipeApi swipeApi, AtomicInteger numSuccessfulReqs, AtomicInteger numFailedReqs) {
